package somasim.malloc
{
	...
	public class Allocator
	{
		...
		private var _sentinelSize :uint = 0;
		
		/** Reference back to the memory manager */
		private var _manager :DomainMemoryManager;
		
		public var freeList :Vector.<AllocationRecord>;
		public var usedList :Vector.<AllocationRecord>;
		
		public function Allocator (manager :DomainMemoryManager, sentinelSize :uint)
		{
			_manager = manager;
		...
		public function allocate (size :uint) :AllocationRecord {
			var realsize :uint = size + 2 * _sentinelSize;
			...
			for (var i :int = 0, len :int = freeList.length; i < len; i++) {
				...
				if (candidate.size > (realsize + 2 * _sentinelSize)) { // additional 2 * sentinel for the new record
					var newrecord :AllocationRecord = new AllocationRecord(candidate.start, realsize, _sentinelSize);
					candidate.setSize(candidate.start + realsize, candidate.size - realsize, _sentinelSize);
		...
		public function onHeapGrowth () :void {
			...
			if (shouldExtendLastFreeElement) {
				...
				lastFreeElement.setSize(lastFreeElement.start, _manager.heap.length - lastFreeElement.start, _sentinelSize);
				return; // EARLY RETURN!
			}
			...
			length = _manager.heap.length - start;
			pushAndSort(freeList, new AllocationRecord(start, length, _sentinelSize));
		}
		...
		private function attemptMerge (list :Vector.<AllocationRecord>, index :uint) :void {
			...
			if (first.nextstart == second.start) {
				...
				first.setSize(first.start, first.size + second.size, _sentinelSize);
