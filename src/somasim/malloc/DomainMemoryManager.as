package somasim.malloc
{
	...
	public class DomainMemoryManager
	{
		...
		private static var _instance :DomainMemoryManager;
		
		/** Debug mode? */
		private var _debug :Boolean;
		
		private var _allocator :Allocator;
		private var _heap :ByteArray;
		...
		public static function get instance () :DomainMemoryManager {
			if (_instance == null) {
				_instance = new DomainMemoryManager(INITIAL_HEAP_SIZE, DEFAULT_DEBUG_OPTION);
			}
			return _instance;
		}
		...
		public function DomainMemoryManager (heapSize :uint, debug :Boolean)
		{
			...
			_heap = new ByteArray();
			_heap.endian = Endian.LITTLE_ENDIAN;
			_heap.length = heapSize;
			
			_allocator = new Allocator(this, (debug ? 4 : 0));
			
			ApplicationDomain.currentDomain.domainMemory = _heap;
		...
		public function get heap () :ByteArray { return _heap; }
		
		/** This is only used in testing, should not be accessed directly. Use allocate() and free() instead. */
		public function get allocator () :Allocator { return _allocator; }
		...
		public function allocate (length :uint) :uint {
			...
			var newRecord :AllocationRecord = _allocator.allocate(length);
			if (newRecord == null) {
				// we ran out of space, or the heap is too fragmented. let's get more space
				growHeap();
				newRecord = _allocator.allocate(length);
			}
		...
		public function reallocate (oldPosition :uint, oldLength :uint, newLength :uint) :uint {
			...
			_heap.position = newPosition;
			_heap.writeBytes(_heap, oldPosition, oldLength);
		...
		public function free (position :uint) :void {
			// in debug mode, check sentinels to make sure nobody wrote over or under their allocation
			if (_debug) {
				var item :AllocationRecord = _allocator.findUsedNodeAt(position);
				if (item != null) {
					checkSentinels(item);
				}
			}
